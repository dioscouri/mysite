 ...
 * @since		1.6
 */
if(!class_exists('MysiteQueryElement'))
{
	
	class MysiteQueryElement extends JObject
	{
...
if(!class_exists('MysiteQuery'))
{
	 ...
	 * @since		1.6
	 */
	class MysiteQuery extends JObject
	{
		...
		public function select($columns)
		{
			...
			if (is_null($this->_select)) {
				$this->_select = new MysiteQueryElement('SELECT', $columns);
			} else {
	    ...
	    {
	        $this->_type = 'delete';
	        $this->_delete = new MysiteQueryElement('DELETE', array(), '');
	        return $this;
	    }
	    ...
	    {
	        $this->_type = 'insert';
	        $this->_insert = new MysiteQueryElement('INSERT INTO', $tables);
	        return $this;
	    }
	    ...
	    {
	        $this->_type = 'update';
	        $this->_update = new MysiteQueryElement('UPDATE', $tables);
	        return $this;
	    }
		...
		public function from($tables)
		{
			if (is_null($this->_from)) {
				$this->_from = new MysiteQueryElement('FROM', $tables);
			} else {
		...
		public function join($type, $conditions)
		{
			...
			$this->_join[] = new MysiteQueryElement(strtoupper($type) . ' JOIN', $conditions);
	    ...
	    public function set($conditions, $glue=',')
	    {
	        if (is_null($this->_set)) {
	            ...
	            $this->_set = new MysiteQueryElement('SET', $conditions, "\n\t$glue ");
	        } else {
		...
		public function where($conditions, $glue='AND')
		{
			if (is_null($this->_where)) {
				$glue = strtoupper($glue);
				$this->_where = new MysiteQueryElement('WHERE', $conditions, "\n\t$glue ");
			} else {
		...
		public function group($columns)
		{
			if (is_null($this->_group)) {
				$this->_group = new MysiteQueryElement('GROUP BY', $columns);
			} else {
		...
		public function having($conditions, $glue='AND')
		{
			if (is_null($this->_having)) {
				$glue = strtoupper($glue);
				$this->_having = new MysiteQueryElement('HAVING', $conditions, "\n\t$glue ");
			} else {
		...
		public function order($columns)
		{
			if (is_null($this->_order)) {
				$this->_order = new MysiteQueryElement('ORDER BY', $columns);
			} else {
