...
class MysiteUrl {
	...
	function &_( $url, $text, $params='', $xhtml=true, $ssl=null, $addItemid='1' ) {
		if ($addItemid == '1') { $url = MysiteUrl::addItemid($url); }
	...
	function popup( $url, $text, $options = array() ) 
	{
		...
		{
		    JHTML::_('behavior.modal', 'a.modal', array('onClose'=>'\function(){mysiteUpdate();}') );
		}
            else
		{
	    ...
	    $browser = Mysite::getClass( 'MysiteBrowser', 'library.browser' );
        if ( $browser->getBrowser() == MysiteBrowser::BROWSER_IE ) 
        {
            // if IE, use 
