...
class MysiteHelperBase extends JObject
{   
	/**
		...
		if (empty($items[$start_datetime][$end_datetime][$period][$select]))
		{
			...
			while ($curdate <= $enddate)
			{
				// set working variables
					$variables = MysiteHelperBase::setDateVariables( $curdate, $enddate, $period );
					$thisdate = $variables->thisdate;
					$nextdate = $variables->nextdate;

				// grab all records
				// TODO Set the query here
					$query = new MysiteQuery();
					$query->select( $select );
	...
	{
        self::includeJQuery();
	    JHTML::_('script', 'jquery-ui-1.7.2.min.js', 'media/com_mysite/js/');
        JHTML::_('stylesheet', 'jquery-ui.css', 'media/com_mysite/css/');
	}
	...
	{
	    JHTML::_('script', 'jquery-1.3.2.min.js', 'media/com_mysite/js/');
	}
	 ...
	 * @access public
	 * @param mixed $data
	 * @param string $title. (default: 'A Mysite Google Chart')
	 * @param string $type. (default: 'Column')
	 * @param int $width. (default: 900)
	 * @param int $height. (default: 250)
	 * @return void
	 */
	function renderGoogleChart($data, $title='A Mysite Google Chart', $type='Column', $width=800, $height=250)
    {
        $title  = JText::_( $title );
