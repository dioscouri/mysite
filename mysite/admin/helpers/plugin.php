...
class MysiteHelperPlugins extends MysiteHelperBase
{
	/**
	 ...
	 */
	function getPluginsWithEvent( $eventName, $folder='Mysite' )
	{
		$return = array();
		if ($plugins = MysiteHelperPlugins::getPlugins( $folder ))
		{
			foreach ($plugins as $plugin)
			{
				if (MysiteHelperPlugins::hasEvent( $plugin, $eventName ))
				{
					$return[] = $plugin;
				}
			}
	 ...
	 * @return array
	 */
	function getPlugins( $folder='Mysite' )
	{
	...
	function hasEvent( $element, $eventName )
	{
		...
		$import 	= JPluginHelper::importPlugin( strtolower('Mysite'), $element->element );
		$dispatcher	= JDispatcher::getInstance();
