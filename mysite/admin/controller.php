...
class MysiteController extends DSCControllerAdmin
{
	...
	*/
	function display($cachable=false)
	{
		...
		$dispatcher->trigger('onBeforeDisplayAdminComponentMysite', array() );
		...
		$dispatcher = JDispatcher::getInstance();
		$dispatcher->trigger('onAfterDisplayAdminComponentMysite', array() );
	...
	function release()
	{
    	...
    	$redirect = "index.php?option=com_mysite&controller=".$this->get('suffix')."&view=".$this->get('suffix')."&task=view&id=".$model->getId()."&donotcheckout=1";
	...
	function cancel()
	{
        if (!isset($this->redirect)) {
            $this->redirect = 'index.php?option=com_mysite&view='.$this->get('suffix');        
        }
	...
	function pagetooltip_switch()
	{
		...
		$msg->link 		= 'index.php?option=com_mysite&view='.$view;
		...
		$config_title = $constant."_disabled";
			...
			JTable::addIncludePath( JPATH_ADMINISTRATOR.DS.'components'.DS.'com_mysite'.DS.'tables'.DS );
			unset($table);
	 ...
	 * Usage: index.php?option=com_mysite&task=doTask&element=pluginname&elementTask=pluginfunction
	 *
	 ...
	 * Usage: index.php?option=com_mysite&task=doTaskAjax&element=pluginname&elementTask=pluginfunction
	 *
	 * @return array(msg=>HTML output from plugin)
	 */
	function doTaskAjax()
	{
		JLoader::import( 'com_mysite.library.json', JPATH_ADMINISTRATOR.DS.'components' );
