...
class MysiteViewBase extends JView 
{
	/**
    ...
    function display($tpl=null) 
    {
        ...
        JLoader::import( 'com_mysite.library.menu', JPATH_ADMINISTRATOR.DS.'components' );
	...
	function displaySubmenu($selected='')
	{
		if (!JRequest::getInt('hidemainmenu') && empty($this->hidemenu)) 
		{
			...
			{
	            JSubMenuHelper::addEntry(JText::_('Dashboard'), JRoute::_('index.php?option=com_mysite&view=dashboard'), $view == 'dashboard' ? true : false );
	            JSubMenuHelper::addEntry(JText::_('Profile'), JRoute::_('index.php?option=com_mysite&view=accounts'), $view == 'accounts' ? true : false );
                JSubMenuHelper::addEntry(JText::_('Order History'), JRoute::_('index.php?option=com_mysite&view=orders'), $view == 'orders' ? true : false);
			}
			JSubMenuHelper::addEntry(JText::_('Shopping Cart'), JRoute::_('index.php?option=com_mysite&view=carts'), $view == 'carts' ? true : false);
		}
	}
    ...
    */
	...
	function _form($tpl='')
	{
		...
		// form
			...
			$form['action'] = $this->get( '_action', "index.php?option=com_mysite&controller={$controller}&view={$view}&task={$task}&id=".$model->getId() );
			$form['validation'] = $this->get( '_validation', "index.php?option=com_mysite&controller={$controller}&task=validate&format=raw" );
